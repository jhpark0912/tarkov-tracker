...
- **해결**: tarkov.dev GraphQL `zones` 필드 기반 퀘스트 좌표 자동 연동 구현
- objective_gps.json fallback 유지
- 커밋: `83621c0`

### ~~2. 탈출 경로 · 잠금 · 스폰 맵 정보~~
- **해결**: MapPositionSyncService로 탈출구/잠금/스폰 위치 동기화 구현
- CoordinateConverter로 게임좌표 → 퍼센트 좌표 변환 + floorId 자동 결정
- 맵 뷰에서 카테고리별 토글 (퀘스트/탈출구/잠금/스폰 ON/OFF)
- 커밋: `83621c0`

### ~~4. 맵 마커 카테고리 토글 UI~~
- **해결**: 맵 뷰 상단에 카테고리별 토글 버튼 구현
- 커밋: `83621c0`
