...
public class SyncResult {
    ...
    private final int spawnsProcessed;
    private final long durationMs;

    @Override
    public String toString() {
        return String.format(
                "동기화 완료 - 딜러: %d, 맵: %d, 아이템: %d, 퀘스트 추가: %d / 갱신: %d / 제거: %d, 탈출구: %d, 잠금: %d, 스폰: %d (%dms)",
                tradersProcessed, mapsProcessed, itemsProcessed,
                questsAdded, questsUpdated, questsRemoved,
                extractsProcessed, locksProcessed, spawnsProcessed, durationMs
        );
    }
}
